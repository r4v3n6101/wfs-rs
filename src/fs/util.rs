...
use goldsrc_rs::{
    texture::{Font, MipTexture, Picture, Rgb},
...
pub fn parse_wad_data(entry: &Entry, level: usize) -> io::Result<Vec<u8>> {
    match entry.ty {
...
fn pic2img<W: Write + Seek>(
    ...
    image::RgbaImage::from_vec(width, height, data)
        .ok_or_else(|| io::Error::new(io::ErrorKind::OutOfMemory, "buffer for img too big"))
        .and_then(|img| {
